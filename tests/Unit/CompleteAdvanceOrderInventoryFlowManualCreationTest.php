...
/**
 * Complete AdvanceOrder Inventory Flow Test - MANUAL CREATION
 *
 * This test validates the EXACT SAME scenario as CompleteAdvanceOrderInventoryFlowTest
 * but using MANUAL OP creation from Filament (use_products_in_orders = true) instead of
 * createAdvanceOrderFromOrders().
 *
 * CRITICAL: Both tests must validate EXACTLY THE SAME expectations because:
 * - Both apply the same inclusion rules (PROCESSED/PARTIALLY_SCHEDULED)
 * - Both consider partially_scheduled flags
 * - Both synchronize pivots via AdvanceOrderCreated event
 * - Both calculate inventory the same way
 *
 * The ONLY difference is the creation method:
 * - Original: createAdvanceOrderFromOrders() - user selects specific orders
 * - This test: Manual creation with use_products_in_orders = true - loads from date range
 *
 * Expected result: ALL validations should pass identically.
 */
class CompleteAdvanceOrderInventoryFlowManualCreationTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function createOp1(): AdvanceOrder
    {
        ...
        // Create AdvanceOrder manually (simulating Filament form submission with use_products_in_orders = true)
        ...
        // Load products automatically from date range (simulating afterCreate hook)
        ...
        $advanceOrderProductRepo = app(\App\Repositories\AdvanceOrderProductRepository::class);
        $advanceOrderRepo = app(\App\Repositories\AdvanceOrderRepository::class);

        $advanceOrderProductRepo->associateProductsWithDefaultQuantity(
            $op1,
            $productsData,
            $advanceOrderRepo
        );
